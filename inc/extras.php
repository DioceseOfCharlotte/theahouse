 ...
 * @return bool
 */
function th_doc_categorized_blog() {

	// Get the categories.
	$all_the_cool_cats = get_transient( 'th_doc_categories' );
	if ( false === $all_the_cool_cats ) {
		...
		set_transient( 'th_doc_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so th_doc_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so th_doc_categorized_blog should return false.
		return false;
	}
}
 ...
 */
function th_doc_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function th_doc_get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
	...
	if ( ! is_numeric( $width ) ) {
		throw new Exception( __( 'Width must be an integer', 'thea-house' ) );
	}

	if ( ! is_numeric( $height ) ) {
		throw new Exception( __( 'Height must be an integer', 'thea-house' ) );
	}

	if ( ! ctype_xdigit( $background_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for background_color', 'thea-house' ) );
	}

	if ( ! ctype_xdigit( $text_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for text_color', 'thea-house' ) );
	}
	...
	$alt = apply_filters( 'th_doc_placeholder_image_alt', __( 'WebDevStudios Placeholder Image', 'thea-house' ) );
 ...
 * @return string
 */
function th_doc_get_placeholder_unsplash( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
		'category'			=> '',
